...
$ gh search -q="org:cli created:>2019-01-01"`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if opts.Query == "" {
				...
				opts.Query = gh_api.PrepareQuery(opts)
			}
...
func runSearch(opts *gh_api.SearchOptions) error {
	results, total, err := gh_api.SearchRepos(opts)
	if err != nil {
		return err
	}
...
func prettyPrint(i int, repo *gh_api.Repository) string {
