...
func FromFile(path string) Option {
	return func(config any) error {
		...
		if err != nil {
			slog.Error("Init config FromFile: readFromFile error", err)
			return err
		}
		return nil
	}
...
func WatchFileChange(hook func(oldConfig any, newConfig any)) Option {
	return func(config any) error {
		...
		if err != nil {
			slog.Error("Init config WatchFileChange: watch error", err)
			return err
		}
		return nil
	}
...
func FromEnv(prefix string) Option {
	return func(config any) error {
		...
		if err != nil {
			slog.Error("Init config FromEnv: readFromEnv error", err)
			return err
		}
		return nil
	}
...
func watch(config any, hook func(oldConfig any, newConfig any)) error {
	...
	viper.OnConfigChange(func(e fsnotify.Event) {
		...
		if err != nil {
			slog.Error("OnConfigChange: deepCopyStruct error", err)
		}
...
func configMustPtrToStruct(config any) bool {
	if !isPtrToStruct(config) {
		slog.Error(ErrConfigNotPtrToStruct.Error(),
			slog.String("config", fmt.Sprintf("%T", config)))
		return false
	}
	return true
}
